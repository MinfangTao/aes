...
inline void myCudaDevAssist(cudaError_t code, int line, bool abort= true)
{
    if (code != cudaSuccess)
    {
...
int aes256_encode_file(FILE *infile,FILE *keyfile,FILE *outfile,bool hexMode)
{
    ...
    cudaSetDevice(0);
    myCudaDevAssist(cudaMemcpyToSymbol(Nk, &Nk_h, sizeof(int), 0, cudaMemcpyHostToDevice), 535, true);
    myCudaDevAssist(cudaMemcpyToSymbol(Nr, &Nr_h, sizeof(int), 0, cudaMemcpyHostToDevice), 543, true);
    myCudaDevAssist(cudaMemcpyToSymbol(Nb, &Nb_h, sizeof(int), 0, cudaMemcpyHostToDevice), 903, true);
    myCudaDevAssist(cudaMemcpyToSymbol(s, &s_h, 256 * sizeof(uint8_t), 0, cudaMemcpyHostToDevice), 920, true);
    myCudaDevAssist(cudaMemcpyToSymbol(ek, &ek_h, 60 * sizeof(uint32_t), 0, cudaMemcpyHostToDevice), 823, true);
    ...
    myCudaDevAssist(cudaMalloc((void **) &devState, RUNNING_THREADS * 16 * sizeof(uint8_t)), 425, true);
    ...
    while (end)
    {
        ...
        myCudaDevAssist(cudaMemcpy(devState, *states, spawn * 16 * sizeof(uint8_t), cudaMemcpyHostToDevice), 426, true);
        myCudaDevAssist(cudaDeviceSynchronize(), 268, true);
        cudaRunner<<<1,spawn>>>(devState);

        myCudaDevAssist(cudaDeviceSynchronize(), 270, true);
        myCudaDevAssist(cudaMemcpy(*states, devState, spawn * 16 * sizeof(uint8_t), cudaMemcpyDeviceToHost), 431, true);
